...
float max_target_speed = 400;                                //Max target speed (100)
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//Declaring Variables for PID Control
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
const byte numChars = 7;
char receivedChars[numChars];   // an array to store the received data
boolean newData = false;
float pid_p_gain = 0;                                       //Gain setting for the P-controller (15)
...
void loop(){
  ...
  angle_gyro = angle_gyro * 0.99998 + angle_acc * 0.00002;                    //Correct the drift of the gyro angle with the accelerometer angle
...
void showNewData() {
    if (newData == true){
      ...
      if(strchr(receivedChars, 'P') > 0){         //If value is a P Variable
        String pValue = receivedChars;            //move character array to string
        pValue.remove(0,1);                       //remove the 'P'
        //Serial.println(pValue);                 //print the value
        pid_p_gain = pValue.toFloat();            //save the value to a global variable
        Serial.println(pid_p_gain);
      }
      if(strchr(receivedChars, 'I') > 0){         //If value is a I Variable
        String iValue = receivedChars;            //move character array to string
        iValue.remove(0,1);                       //remove the 'I'
        //Serial.println(iValue);                 //print the value
        pid_i_gain = iValue.toFloat();            //save the value to a global variable
        Serial.println(pid_i_gain);               //print back to node-red      
      }
      if(strchr(receivedChars, 'D') > 0){         //If value is a D Variable
        String dValue = receivedChars;            //move character array to string
        dValue.remove(0,1);                       //remove the 'D'
        //Serial.println(dValue);                 //print the value
        pid_d_gain = dValue.toFloat();            //save the value to a global variable
        Serial.println(pid_d_gain);               //print back to node-red
      }
      else{
        String strControl = receivedChars;        //move character array to string
        control_byte = strControl.toInt();        //convert to int and save the value to a global variable
        //Serial.println(control_byte);           //print control value back to node-red
